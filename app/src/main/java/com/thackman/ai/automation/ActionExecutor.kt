...
class ActionExecutor(private val context: Context) {
    ...
    fun execute(rawCommand: String): AutomationResult {
        ...
        if (cmd.contains("batterie") || cmd.contains("énergie") || cmd.contains("niveau de batterie")) {
            val battery = hardware.getBatteryLevel()
            return AutomationResult(
                executed = true,
                actionName = "HARDWARE_BATTERY",
                responseMessage = "Niveau d'énergie du système : $battery%."
            )
        }
